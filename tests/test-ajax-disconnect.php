<?php
/**
 * R2C_Ajax::handle_disconnect() — FR-07's guarantee that local credential
...
class AjaxDisconnectTest extends TestCase {
	...
	public function test_disconnect_with_no_local_key_skips_the_remote_call_and_still_succeeds() {
		Functions\when( 'get_option' )->justReturn( '' ); // R2C_Options::get_api_key() -> ''
	...
	public function test_remote_disconnect_success_clears_local_state_with_no_warning() {
		...
		$this->assertContains( \R2C_Options::API_KEY, $deleted );
		$this->assertContains( \R2C_Options::TENANT_ID, $deleted );
		$this->assertContains( \R2C_Options::SITE_ORIGIN, $deleted );
		$this->assertContains( \R2C_Options::CACHED_THEME, $deleted );
	}
